...
# FZFS: fuzzy-finder helpers.

# -----------------------------------------------------------------------------
# UI constants
# -----------------------------------------------------------------------------

# Skip plugin setup if no fzf binary is available.
if [ -n "${FZF_BIN:-}" ] && __dot_has "$FZF_BIN"; then
  :
elif [ -n "${FZF_CMD:-}" ] && __dot_has "$FZF_CMD"; then
  :
elif ! __dot_has fzf; then
  return 0
fi
...
# -----------------------------------------------------------------------------
# Tool detection (static resolution)
# -----------------------------------------------------------------------------

# We detect tools once at startup to keep the script feeling "Instantaneous".
if __dot_has eza; then TOOL_LS="eza"; elif __dot_has exa; then TOOL_LS="exa"; else TOOL_LS="ls"; fi
if __dot_has rg; then TOOL_GREP="rg"; else TOOL_GREP="grep"; fi
if __dot_has bat; then TOOL_CAT="bat"; else TOOL_CAT="cat"; fi
if __dot_has fd; then HAS_FD=1; else HAS_FD=0; fi
if __dot_has delta; then HAS_DELTA=1; else HAS_DELTA=0; fi
...
_fzfs_copy() {
  if __dot_has pbcopy; then
    printf '%s' "$1" | pbcopy
  elif __dot_has xclip; then
    printf '%s' "$1" | xclip -selection clipboard
  elif __dot_has xsel; then printf '%s' "$1" | xsel --clipboard --input; fi
}
...
_fzfs_resolve_self() {
  ...
  if __dot_has readlink && readlink -f "$src" >/dev/null 2>&1; then
    readlink -f "$src"
  else
    dir=
...
if [ ! -r "$FZFS_SCRIPT_DIR/callbacks.sh" ] && [ -n "${DOTFILES_DIR:-}" ] && [ -r "$DOTFILES_DIR/config/plugins/fzf/callbacks.sh" ]; then
  FZFS_SCRIPT_DIR="$DOTFILES_DIR/config/plugins/fzf"
  FZFS_SCRIPT_PATH="$FZFS_SCRIPT_DIR/init.sh"
fi
...
# -----------------------------------------------------------------------------
# Callback handlers (preview engine)
# -----------------------------------------------------------------------------
# shellcheck source=/dev/null
. "$FZFS_SCRIPT_DIR/callbacks.sh"
. "$FZFS_SCRIPT_DIR/bindings.sh"

# -----------------------------------------------------------------------------
# Interactive UI (controllers)
# -----------------------------------------------------------------------------

# Unified FZF loop for most modes.
_fzfs_ui_search() {
  [ -n "${ZSH_VERSION:-}" ] && setopt localoptions shwordsplit
  mode="$1" base="$2" edit="$3" self_q
  self_q="$(_fzfs_quote "$FZFS_SCRIPT_PATH")"
  base_path=
  base_path="$(_fzfs_expand_path "$base")"
  src_cmd="" preview_cmd="FZFS_BASE=$(_fzfs_quote "$base_path") sh $self_q --internal-preview {}"
  fzf_mode_opts="--multi"
  show_hidden="${FZFS_SHOW_HIDDEN:-1}"
  show_hidden_expr='${FZFS_SHOW_HIDDEN:-1}'
  use_relative=0
  ...
  reload_cmd="$src_cmd"
  binds="${BIND_YANK}:execute-silent(sh $self_q --internal-copy {})"
  ...
  friendly_toggle='FZFS_FRIENDLY=$((1-${FZFS_FRIENDLY:-1}))'
  hidden_toggle='FZFS_SHOW_HIDDEN=$((1-${FZFS_SHOW_HIDDEN:-1}))'
...
_fzfs_ui_branch() {
  ...
  self_q=
  self_q="$(_fzfs_quote "$FZFS_SCRIPT_PATH")"
  c_loc="sh $self_q --internal-gen-branches local"
  c_rem="sh $self_q --internal-gen-branches remote"
  c_fet="git fetch --all --prune >/dev/null 2>&1"
  binds="ctrl-l:change-prompt(Local> )+reload($c_loc)"
