...
# Re-entrancy guard
[ -n "${__DOTFILES_CORE_LOADED:-}" ] && return 0
__DOTFILES_CORE_LOADED=1
...
__dot_load_plugin() {
  plugin="$1"
  plugin_dir="$DOTFILES_MODULES_DIR/$plugin"
...
rdotfiles() {
  ...
  __dot_log "dotfiles: rdotfiles not found (run lib/install/link.sh)"
  return 1
}
